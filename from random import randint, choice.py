...
def creer_tableau_plein(lignes,colonnes):
    ...
    full_tab = [[1,2,3,4,5,6,7,8,9],
                ...
                [6,7,8,9,1,2,3,4,5],
                [9,1,2,3,4,5,6,7,8]]
#renvoie un tableau de Sudoku et permet de creer un peu de hasard

    for _ in range(0,lignes,3):
        c = choice(list_of_choices) #permet de choisir au hasard les elements de la liste entre 0 3 et 6 => ça ne sera jamais les meme chiffres si on redemarre la partie
        list_of_choices.remove(c)#ce qui est choisit au hasard est supprimer DE LA LISTE
...
def update_texte():#permet de saisir des chiffres dans les cases 
    global POSITION_SOURIS
    item = canvas.find_closest(*POSITION_SOURIS)[0]#recupere la case la plus proche pour pouvoir saisir du texte 
    texte=my_entry.get()#permet d'ecrire un chiffre
    print(texte)
    print(item)
    canvas.itemconfigure(item,text=texte)#permet sont affichage dans la fenetre
